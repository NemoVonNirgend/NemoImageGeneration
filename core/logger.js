/**
 * NemoImageGeneration - Centralized Logging Utility
...
class Logger {
    constructor(prefix = 'NemoImageGeneration', level = CONSTANTS.LOG_LEVELS.INFO) {
        this.prefix = prefix;
        this.level = level;
...
if (isDebugEnabled()) {
    ...
    console.log('[NemoImageGeneration] Debug mode enabled');
}

// Export utility to toggle debug mode at runtime
// Usage: window.NemoImageGenerationLogger.enableDebug() or .disableDebug()
if (typeof window !== 'undefined') {
    window.NemoImageGenerationLogger = {
        enableDebug: () => {
            logger.setDebugMode(true);
            try { localStorage.setItem('nemoPresetExtDebug', 'true'); } catch (e) {}
            console.log('[NemoImageGeneration] Debug mode enabled (persisted to localStorage)');
        },
        disableDebug: () => {
            logger.setDebugMode(false);
            try { localStorage.removeItem('nemoPresetExtDebug'); } catch (e) {}
            console.log('[NemoImageGeneration] Debug mode disabled');
        },
        getLogger: () => logger
    };
}
